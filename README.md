...
| `thinkingBudgetTokens` | int \| null | `null` | Extended thinking 预算 token 数。≥512 或 null=禁用；设置后 `max_tokens` 自动调整为 `thinkingBudgetTokens + 1024` 最小值 |
...
7. **`_stream_events.py`** — provider 协议契约。5 个 dataclass 是两个 transport 共同说的语言 + thinking 事件。
8. **`_provider_anthropic.py`** — Anthropic transport：SDK SSE 事件 → 5 个 dataclass + 原生 thinking 支持。
9. **`_provider_openai.py`** — OpenAI transport：同样翻译到 5 个 dataclass，顺带做消息格式转换 + reasoning_content 支持。
...
13. **`session/`** — 会话持久化。transcript (.jsonl) 写入/读取/reset + sessions.json 索引 + 8KB 截断 + store-first 初始化。理解 OpenClaw 的 session 恢复机制。
14. **`cli.py`** — 给人看的部分。理解 `on_event` 回调如何把"loop 内部状态"暴露给"渲染层"，包括 thinking dim 渲染。
...
│   ├── __init__.py
│   ├── __main__.py             入口；--config 加载 JSON5 配置文件；构建 registry + LoopConfig + thinking 配置
│   ├── config/                 配置系统
...
│   ├── _stream_events.py       5 个共享 StreamEvent dataclass + thinking 事件（provider 协议契约）
│   ├── _provider_anthropic.py  Anthropic Messages API transport + 原生 thinking 支持
│   ├── _provider_openai.py     OpenAI Chat Completions transport + 消息格式转换 + 图片块转换 + reasoning_content
│   ├── provider.py             路由层：switch(api) → 对应 transport
│   ├── compact.py              上下文压缩：estimate_tokens / summarize_history / compact_if_needed
│   ├── loop.py                 agent_loop（spine）；LoopConfig 含 api + base_url + model_input + context_* + image_model + thinking_budget
│   ├── cli.py                  rich REPL + 工具调用面板渲染 + 压缩提示 + 图片事件渲染 + thinking dim 渲染 + 会话管理
│   └── session/                会话持久化模块
...
│       ├── store.py            sessions.json 管理：load/save/update/list + store-first 初始化
│       ├── transcript.py       JSONL 读写：Writer append/resume/reset / Reader load_history
