...
6. **`tools.py`** — 模型能干什么。看 `Tool` 形状、4 个内置工具、`dispatch` 永不抛异常的契约。
7. **`images.py`** — 图片怎么处理。`parse_image_refs` 检测引用 → `load_image` 加载 → `describe_image` 双路径架构。
8. **`_stream_events.py`** — provider 协议契约。5 个 dataclass 是两个 transport 共同说的语言 + thinking 事件。
9. **`_provider_anthropic.py`** — Anthropic transport：SDK SSE 事件 → 5 个 dataclass + 原生 thinking 支持。
10. **`_provider_openai.py`** — OpenAI transport：同样翻译到 5 个 dataclass，顺带做消息格式转换 + reasoning_content 支持。
11. **`provider.py`** — 路由层：`switch(api)` 派发给正确的 transport，对外只暴露一个 `stream_response`。
12. **`compact.py`** — 上下文压缩：`estimate_tokens` → `compact_if_needed` → `summarize_history`。
13. **`loop.py`** — 把上面全部粘起来。这一步最关键，看完你就懂 agent 了。
14. **`session/paths.py`** — Session 路径解析。按 agent 隔离的 session 存储结构。
15. **`session/store.py`** — sessions.json 管理。
16. **`session/transcript.py`** — JSONL 转录文件读写。
17. **`session/truncate.py`** — tool_result 截断。
18. **`cli.py`** — 给人看的部分。理解 `on_event` 回调如何把"loop 内部状态"暴露给"渲染层"。
19. **`__main__.py`** — 入口装配。配置加载 → 模型解析 → LoopConfig 构建 → 启动 REPL。
...
├── .python-version           3.11
├── workspace/
│   └── nano-openclaw.json5   项目级配置模板（复制到项目根目录或 .openclaw/ 使用）
