...
7. **`loop.py`** — 把上面全部粘起来。这一步最关键，看完你就懂 agent 了。
8. **`cli.py`** — 给人看的部分。理解 `on_event` 回调如何把"loop 内部状态"暴露给"渲染层"。
9. **`__main__.py`** — 入口装配。
...
│   ├── __init__.py
│   ├── __main__.py             入口；--api 选 provider，构建 registry 后交给 cli.repl
...
│   ├── loop.py                 agent_loop（spine）；LoopConfig 含 api 字段
