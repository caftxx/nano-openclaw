...
├── LICENSE                   MIT
├── pyproject.toml            uv 管理；anthropic + openai + rich + pillow + json5 + pydantic + mcp + ddgs + readability-lxml
├── uv.lock                   锁定版本
├── .python-version           3.11
...
│   ├── tools.py              Tool / ToolRegistry / 内置工具（含 web_search/web_fetch 条件注册）
│   ├── web_search.py         DuckDuckGo 网页搜索工具（缓存 + 外部内容包装）
│   ├── web_fetch.py          URL 内容抓取工具（readability 提取 + markdown 转换 + SSRF 防护）
│   ├── ssrf_guard.py         SSRF 防护（两阶段 hostname 验证：预 DNS + 后 DNS）
│   ├── external_content.py   外部内容安全包装（<EXTERNAL_UNTRUSTED_CONTENT> + LLM token 清洗）
│   ├── approvals/            审批门禁模块（危险命令确认，对齐 openclaw exec-approvals）
│   │   ├── __init__.py       公开接口
│   │   ├── exec_approvals.py 加载 exec-approvals.json + 多层策略解析（镜像 resolveExecApprovalsFromFile）
│   │   ├── types.py          ApprovalDecision / ApprovalRequest / ApprovalPolicy / AllowlistEntry
│   │   ├── policy.py         策略评估器（危险模式匹配 + allowlist 评估）
│   │   ├── manager.py        审批管理器（请求生命周期 + per-agent allowlist 持久化）
│   │   └── ui.py             Rich 审批提示 UI
│   ├── images.py             parse_image_refs / load_image / describe_image（双路径）
│   ├── mcp/                  MCP 服务器集成（连接管理 + 工具注册）
│   │   ├── __init__.py       公开接口
│   │   ├── runtime.py        McpRuntime（连接管理 + 工具调用：stdio/SSE/streamable-http）
│   │   └── materialize.py    materialize_mcp_tools（MCP 工具 → nano-openclaw Tool）
...
└── tests/
    ├── test_approvals.py       审批门禁单测（策略评估、allowlist 持久化、on-miss 语义验证）
    ...
    ├── test_memory_tools.py    Memory 工具单测（memory_get、memory_search、上下文窗口匹配、停用词过滤）
    ├── test_mcp_runtime.py     MCP 运行时单测（传输协议处理、连接管理）
    ├── test_cancellation.py    Agent turn 取消单测（CancellationToken、流式取消、工具派发取消）
