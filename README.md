...
两者相互独立、可任意搭配；凭据缺失或服务不可用时自动回退浏览器原生，互不影响。识别与合成是阿里云智能语音交互下**两个独立产品**——其中流式语音合成需开通**商用版**（不支持试用）。合成的回退链是 **流式 →（不可用/失败）RESTful 代理合成 →（再失败）浏览器本地**：流式不可用或失败时自动改用阿里云 **RESTful 语音合成**（标准「语音合成」产品，试用版亦可用），经后端 `/api/voice/tts` 代理（浏览器永不接触 appkey/AK/SK），本会话内记住回退结果不每轮重试；RESTful 仍失败才退浏览器本地并提示真实原因。配置见 [语音（阿里云语音）配置](#语音阿里云语音配置)。
