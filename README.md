...
12. **`tools.py`** — 模型能干什么。看 `Tool` 形状、5 个内置工具、`dispatch` 永不抛异常的契约。包含 `session_status` 工具用于查询日期时间和会话上下文。
...
├── LICENSE                   MIT
├── pyproject.toml            uv 管理；anthropic + openai + rich + pillow + json5 + pydantic + mcp
├── uv.lock                   锁定版本
├── .python-version           3.11
