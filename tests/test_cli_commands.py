...
def test_commands_help_hides_plugin_commands_with_explicit_empty_load():
    registry = build_core_registry()
    load_plugins(PluginsConfig(load=[]), registry, NanoOpenClawConfig())

    help_text = _commands_help(registry, LoopConfig())

    assert "/subagents" not in help_text
    assert "/active-memory" not in help_text
    assert "/dreaming" not in help_text
